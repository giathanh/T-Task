<x-admin-layout :title="$project->exists ? 'Chỉnh sửa dự án' : 'Thêm dự án'">
    <form method="POST" action="{{ $project->exists ? route('admin.projects.update', $project) : route('admin.projects.store') }}" class="flex w-full max-w-3xl flex-col gap-6 rounded-2xl border border-outline-variant bg-surface-container-lowest p-6">
        @csrf
        @if ($project->exists)
            @method('PUT')
        @endif
