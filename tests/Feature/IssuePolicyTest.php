...
class IssuePolicyTest extends TestCase
{
    ...
    public function test_allows_create_for_a_member_with_any_role(ProjectRole $role): void
    {
        $user = User::factory()->create();
        $project = Project::factory()->create();
        $project->members()->attach($user, ['role' => $role->value]);

        $this->assertTrue((new IssuePolicy)->create($user, $project));
    }

    public function test_forbids_create_for_a_user_who_is_not_a_project_member(): void
    {
        $user = User::factory()->create();
        $project = Project::factory()->create();
